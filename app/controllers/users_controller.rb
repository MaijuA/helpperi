class UsersController < ApplicationController
  ...
  def index
    ...
    if current_user
      ...
      @user_performer_buying_posts = Post.where(doer_id:current_user.id).valid.not_rated.buying # ne joissa ratings.count < 2
      @user_performer_selling_posts = Post.where(doer_id:current_user.id).valid.not_rated.selling # ne joissa ratings.count < 2
  ...
  def history
    if current_user
      ...
      params[:page3] = 1 if params[:page3] == ''
      @user_expired_posts = current_user.posts.active.expired.paginate(:page => params[:expired_page], :per_page => 5)
      @user_performed_buying_posts = current_user.posts.rated.buying
      @user_performed_selling_posts = current_user.posts.rated.selling
      @user_performed_buying_tasks = Post.where(doer_id:current_user.id).rated.buying
      @user_performed_selling_tasks = Post.where(doer_id:current_user.id).rated.selling
