module ApplicationHelper
  def users_index_message_count  # counts all unread messages in users/index
    Conversation.where(post_id:current_user.posts.ids).user_messages(current_user.id).unread_by(current_user).count +
      Conversation.where(post_id:(current_user.tasks.ids)).user_messages(current_user.id).unread_by(current_user).count +
  ...
  def users_index_candidate_count # counts all changes in candidates in users/index
    Candidate.where(post_id:current_user.posts.ids).where(denied:false).unread_by(current_user).count +
