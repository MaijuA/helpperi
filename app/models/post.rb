class Post < ActiveRecord::Base
  ...
  scope :rated, -> { where(deleted:false).where(id:(Post.joins(:ratings).select('ratings.post_id, COUNT(ratings.post_id)').having('COUNT(ratings.post_id) > 1').group('ratings.post_id'))) }
